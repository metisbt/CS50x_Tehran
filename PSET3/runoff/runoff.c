...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            int candidate_temp = preferences[i][j];
            if (candidates[candidate_temp].eliminated == false)
            {
                candidates[candidate_temp].votes++;
                break;
            }
            else
            {
...
bool print_winner(void)
{
    int h_win = round(voter_count / 2);
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes > h_win)
        {
            printf("%s\n", candidates[i].name);
            return true;
        }
